...
class RealsenseCalibration:
        ...
        if self.pattern == 'circle':
            while self.found < self.num:  # Here, self.num is the amount of pictures to be taken
                ...
                if ret:
                    ...
                    # Save calibration image prior to drawing the found circles
                    _filename = self.path + "rs_" + str(self.found) + ".jpg"
                    cv2.imwrite(_filename, _img_gray)
                    ...
                    _filename = self.path + "rs_grid_" + str(self.found) + ".jpg"
                    cv2.imwrite(_filename, _img_with_keypoints)

                    # Increment found
                    print("Status realsense: %d out of %d pictures taken." % (self.found, self.num))
                ...
                try:
                    ...
                    cv2.imshow("img", _img_gray)  # display
                    cv2.waitKey(100)
        else:
            while self.found < self.num:  # Here, self.num is the amount of pictures to be taken
                _img_data, _ = self.rs.get_frame()
                ...
                if ret:
                    ...
                    # Save calibration image prior to drawing the found circles
                    _filename = self.path + "rs_cb_" + str(self.found) + ".jpg"
                    cv2.imwrite(_filename, _img_gray)
                    ...
                    # Save the calibration images - the grayscale image and the equivalent image with the circle grid marked
                    _filename = self.path + "rs_cb_grid_" + str(self.found) + ".jpg"
                    cv2.imwrite(_filename, _img_with_keypoints)
    ...
    def calibrate(self):
        if self.pattern == 'circle':
            # Check for files with the circle grid pattern
            _list_file_names = [f for f in os.listdir(self.path) if f.startswith("rs_grid_")]
            index = []
            print(_list_file_names)
            for file in _list_file_names:
                index.append(int(file[8:-4]))
        else:
            # Check for files with the chessboard pattern
            _list_file_names = [f for f in os.listdir(self.path) if f.startswith("rs_cb_grid_")]
            index = []
            print(_list_file_names)
            for file in _list_file_names:
                index.append(int(file[11:-4]))
        # Pick a random number of images
        ...
        if self.pattern == 'circle':
            for i in range(self.num):
                ...
                _filename = self.path + "rs_" + str(index[i]) + ".jpg"
                _img_gray = cv2.imread(_filename, cv2.IMREAD_GRAYSCALE)

                # Find the circle grid as before
                ret, _corners = cv2.findCirclesGrid(_img_gray, (self.pattern_rows, self.pattern_columns),
                                                    flags=cv2.CALIB_CB_ASYMMETRIC_GRID  # ,
                                                    # blobDetector=self.blobDetector
                                                    )  # Find the circle grid
            ...
            for i in range(self.num):
                # Load previously taken pictures
                _filename = self.path + "rs_cb_" + str(index[i]) + ".jpg"
                _img_gray = cv2.imread(_filename, cv2.IMREAD_GRAYSCALE)
                ...
                _corners_refined = cv2.cornerSubPix(_img_gray, _corners, (5, 5), (-1, -1), self.criteria)
        ...
        try:
            #ret, self.mtx, self.dist, rvecs, tvecs = cv2.calibrateCamera(self.objpoints, self.imgpoints, _img_gray.shape[::-1], None, None)
            ret, self.mtx, self.dist, self.rvecs, self.tvecs, stdDevInt, stdDevExt, self.per_view_errors = cv2.calibrateCameraExtended(
                self.objpoints, self.imgpoints, _img_gray.shape[::-1], None, None
            )
        ...
        if ret:
            ...
            print(f"RealSense reprojection error: {reprojection_error_av} +/- {reprojection_error_sd}\n")
            print('RealSense calibration done.')
            ...
            ax.axhline(reprojection_error_av+reprojection_error_sd, alpha=0.5, dashes=(10, 10),
                       label='Standard Deviation: +/- %.2f' % reprojection_error_sd, color='red')
            ax.axhline(reprojection_error_av - reprojection_error_sd, alpha=0.5, dashes=(10, 10),
                       color='red')
            ax.legend()
            if self.pattern == 'circle':
                fig.savefig(self.path + 'rs_reprojection_error_' + str(self.num) + '.png')
            else:
                fig.savefig(self.path + 'rs_cb_reprojection_error_' + str(self.num) + '.png')

        if self.pattern == 'circle':
            for k in range(len(self.objpoints)):
                # Load previously taken pictures
                _filename = self.path + "rs_" + str(_list_good_images[k]) + ".jpg"
                _img_gray = cv2.imread(_filename, cv2.IMREAD_GRAYSCALE)
                ...
                if ret:
                    ...
                    for im_point in im_points:
                        _img_with_keypoints = cv2.circle(_img_with_keypoints,
                                                         (int(im_point[0][0]), int(im_point[0][1])), 6, (0, 0, 255), 2)
                    #_img_with_keypoints = cv2.drawChessboardCorners(_img_with_keypoints,
                    #                                                (self.pattern_rows, self.pattern_columns), im_points,
                    #                                                ret)
                    ...
                    _filename = self.path + "rs_reprojected_" + str(_list_good_images[k]) + ".jpg"
                    cv2.imwrite(_filename, _img_with_keypoints)
        else:
            for k in range(len(self.objpoints)):
                # Load previously taken pictures
                _filename = self.path + "rs_cb_" + str(_list_good_images[k]) + ".jpg"
                _img_gray = cv2.imread(_filename, cv2.IMREAD_GRAYSCALE)
                ...
                if ret:
                    ...
                    # Save calibration image with the circle grid marked and the reprojection
                    _filename = self.path + "rs_cb_reprojected_" + str(_list_good_images[k]) + ".jpg"
                    cv2.imwrite(_filename, _img_with_keypoints)
        ...
        if self.pattern == 'circle':
            with open(self.path + "calibration_rs_" + str(self.num) + ".yaml", "w") as file:
                yaml.dump(data, file)
        else:
            with open(self.path + "calibration_rs_cb.yaml", "w") as file:
                yaml.dump(data, file)
